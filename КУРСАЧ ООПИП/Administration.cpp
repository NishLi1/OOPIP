...
void Administration::generateReport() {
    ...
    if (choice == 'y' || choice == 'Y') {
        system("cls");
        printBorder();
        printCenteredInBox("Введите имя текстового файла (например: report.txt): ");
        printEmptyLineInBox();
        printBorder();
        printCentered("> ");
        std::string filename;
...
void Administration::viewRecommendation() {
    system("cls");
    printBorder();
    printCenteredInBox("=== РЕКОМЕНДАЦИЯ ОТ СПЕЦИАЛИСТА ===");
    printEmptyLineInBox();

    if (g_recommendation.empty()) {
        printCenteredInBox("Нет рекомендаций от специалиста");
    }
    else {
        std::stringstream ss(g_recommendation);
        std::string line;
...
void Administration::AdminMenu() {
    std::vector<std::string> items = {
        ...
        "Просмотреть рекомендацию от специалиста",
        "Вывести отчёт",
        "Назад"
    };
    ...
    while (true) {
        int choice = menu(items, "=== Руководитель  : управление записями ===");
        switch (choice) {
        case 0: {
