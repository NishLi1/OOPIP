...
void WorkFile::saveUsers() {
    ...
    for (auto& u : g_users) {
        ...
        ofs.write((char*)&u.approved, sizeof(u.approved));
        writeString(ofs, u.name);
        writeString(ofs, u.education);
        ofs.write((char*)&u.experience, sizeof(u.experience));
    }
}
...
void WorkFile::loadUsers() {
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        ifs.read((char*)&u.approved, sizeof(u.approved));
        u.name = readString(ifs);
        u.education = readString(ifs);
        ifs.read((char*)&u.experience, sizeof(u.experience));
        g_users.push_back(u);
    }
}
...
void WorkFile::saveRecommendation() {
    std::ofstream ofs("recommendation.txt");
    if (ofs) {
        ofs << g_recommendation;
    }
}

void WorkFile::loadRecommendation() {
    std::ifstream ifs("recommendation.txt");
    if (ifs) {
        std::stringstream buffer;
        buffer << ifs.rdbuf();
        g_recommendation = buffer.str();
    }
}