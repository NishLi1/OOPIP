...
void BasicStaff::showMenu() {
    ...
    while (true) {
        ...
        case 5: showTask(); break;
        case 6: return;
        }
    }
}

void BasicStaff::addTask(){
    std::vector<std::string> typeItems = {
        "Текст — Описание задачи",
        "Количество / Остаток (целое)",
        "Время (часы) (вещественное)",
        "Отмена"
    };
    int t = menu(typeItems, "=== Выберите тип задачи ===");
    if (t == 3 || t < 0) {
        return;
    }
    ...
    if (t == 0) { 
        system("cls");
        printBorder();
        printCenteredInBox("=== Добавление: Описание задачи ===");
        printEmptyLineInBox();
        printCentered("Введите описание задачи: ");
        std::string val;
        std::getline(std::cin, val);
        if (val.empty()) std::getline(std::cin, val); 
        tasksStr.add(val);
        ...
        system("cls");
        printBorder();
        printCenteredInBox("=== Добавление: Количество / Остаток ===");
        printEmptyLineInBox();
        printCentered("Введите целое число: ");
        std::string val;
        std::getline(std::cin, val);
        if (val.empty()) std::getline(std::cin, val);
        try {
            ...
            system("cls");
            printBorder();
            printCenteredInBox("Ошибка: неверный ввод целого.");
            printEmptyLineInBox();
            printBorder();
            printCentered("Нажмите любую клавишу...");
            _getch();
        ...
        system("cls");
        printBorder();
        printCenteredInBox("=== Добавление: Время (часы)===");
        printEmptyLineInBox();
        printCentered("Введите число: ");
        std::string val;
        std::getline(std::cin, val);
        if (val.empty()) std::getline(std::cin, val);
        try {
            ...
            system("cls");
            printBorder();
            printCenteredInBox("Ошибка: неверный ввод числа.");
            printEmptyLineInBox();
            printBorder();
            printCentered("Нажмите любую клавишу...");
            _getch();
...
void BasicStaff::showTask() {
    system("cls");
    printBorder();
    printCenteredInBox("=== ВСЕ ЗАДАЧИ ===");
    printEmptyLineInBox();
    ...
    if (tasksStr.size() > 0) {
        ...
        printCenteredInBox("Текстовых задач нет");
        printEmptyLineInBox();
    }
    ...
    if (tasksInt.size() > 0) {
        ...
        printCenteredInBox("Задач по количеству нет");
        printEmptyLineInBox();
    }
    if (tasksDouble.size() > 0) {
        tasksDouble.show();
    }
    else {
        printCenteredInBox("Задач по времени нет");
        printEmptyLineInBox();
    }
